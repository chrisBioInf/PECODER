...
def overlap_index(G, fuzzy_cutoff: int, domain_1_atoms: list, domain_2_atoms: list) -> float:
    ...
    if (fuzzy_cutoff == 0):
        ...
        for atom1, atom2 in product(domain_1_atoms, domain_2_atoms):
            shortest_path = nx.shortest_path(G, source=node_dict.get(atom1), target=node_dict.get(atom2))
            if len(shortest_path)-1 <= fuzzy_cutoff:
                shared_atoms.add(atom1)
                shared_atoms.add(atom2)
            
    overlap = round(len(shared_atoms) / (len(domain_1_atoms) + len(domain_2_atoms)), 2)    
    return overlap, shared_atoms
